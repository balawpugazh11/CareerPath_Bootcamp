...
export default function BootcampDetail() {
  const { id } = useParams();
  const { bootcamps, loading } = useBootcamps();

  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <p className="text-slate-500 text-lg">Loading bootcamp...</p>
      </div>
    );
  }

  const bootcamp = bootcamps.find((item) => item.id === id);

  if (!bootcamp) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-4">
        <h1 className="text-3xl font-bold text-slate-800">Bootcamp Not Found</h1>
        <p className="text-slate-500">The bootcamp you're looking for doesn't exist or has been removed.</p>
        <Link to="/" className="text-cyan-600 hover:text-cyan-800 font-medium underline">← Back to Bootcamps</Link>
      </div>
    );
  }
